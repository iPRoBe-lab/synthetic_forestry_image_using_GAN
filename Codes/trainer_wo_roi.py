...
class Trainer():
    ...
    def build_model(self):
        if self.args.model == "DCGAN":
            from DCGAN_adjusted_v13_wo_ROI import Generator, Discriminator
        # Initialize weights
        def weights_init(m):
    ...
    def train(self):
        ...
        for epoch in range(start, self.num_epoch): 
            ...
            for step in range(0, steps_per_epoch):
                ...
                d_out_real = self.D(real_images, values)
                ...
                fake_images = self.G(z, values)
                d_out_fake = self.D(fake_images.detach(), values)
                ...
                z = Variable(torch.randn(current_batch_size, self.latent_dim).to(self.device))
                fake_values = Variable(((self.max_value-self.min_value)*torch.rand((current_batch_size,1))+self.min_value).to(self.device))
                fake_images = self.G(z, fake_values)

                # Compute loss with fake images
                g_out_fake = self.D(fake_images, fake_values)  # batch x n
                if self.args.adv_loss == 'WGAN-GP':
                    g_loss_fake = - g_out_fake.mean()
                elif self.args.adv_loss == 'HingeGAN':
                    g_loss_fake = - g_out_fake.mean()
            ...
            if (epoch + 1) % self.args.sample_step == 0:
                self.G.eval()
                fake_images = self.G(z_test, z_test_values).detach()
